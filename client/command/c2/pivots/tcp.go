...
type TCPPivot struct {
	Options struct {
		...
		LPort int    `long:"lport" short:"l" description:"listener TCP listen port" default:"9898"`
...
func (tp *TCPPivot) Execute(args []string) (err error) {
	...
	if err != nil {
		return log.Errorf("Failed to start TCP pivot listener: %s", err)
	}

	log.Infof("Listening on tcp://%s", address)
	return
}
