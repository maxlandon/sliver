...
// ActiveNetwork - Returns an interface representing a miniature standard net package,
// which transparently offers a way to dial to or listen on any interface reachable by:
// - the server when no session is active.
// - the implant host when a session is currently used.
// Please read carefully the functions documentation strings before using.
func ActiveNetwork(sessionID uint32) (net Net, err error) {

	// If no Session ID is provided, this means there is simply no
	// active session, and thus we return the server "net" package.
	if sessionID == 0 {
		return serverNetwork, nil
	}

	// Else, try to find the corresponding session
	comm := Comms.GetBySession(sessionID)

	// If no session Comm is found, this means the session
	// doesn't have a Comm subsystem running, and we must
	// return an error to whatever the caller tool was up to.
	if comm == nil {
		return nil, fmt.Errorf("session %d has no running Comm subsystem", sessionID)
	}

	// Else return the virtual session network
	return comm, nil
}
...
func SetCommString(session *core.Session) string {
	...
	route, err := ResolveBySession(session)
...
func SetHandlerCommString(host string, sess *core.Session) string {

	if sess == nil {
		return "[server] " + host
	}

	ip, err := url.Parse("scheme://" + host)
	if ip == nil {
		return "[server] " + host
	}

	route, err := ResolveBySession(sess)
	if err != nil || route == nil {
		return "[server] " + host
	}
