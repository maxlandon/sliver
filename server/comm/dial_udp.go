...
// DialUDP - Get a working UDP (packet) connection to a destination host reachable from the implant.
func (comm *Comm) DialUDP(network, host string) (conn net.PacketConn, err error) {
	return comm.DialContextUDP(context.Background(), network, host)
}
