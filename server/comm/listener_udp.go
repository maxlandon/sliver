...
func (c *Comm) ListenPacket(network, host string) (conn net.PacketConn, err error) {
	return c.ListenUDP(network, host)
}

// ListenUDP - Listen for UDP packets on one of the session's host interfaces.
func (c *Comm) ListenUDP(network, host string) (conn net.PacketConn, err error) {
	...
	// Get an abstracted UDP listener. This is blocking until when the
	// listener receives a stream from the Comm system to pass data into.
	conn, err = newListenerUDP(addr, c)
	if err != nil {
		return nil, fmt.Errorf("comm listener: %s", err.Error())
	}
