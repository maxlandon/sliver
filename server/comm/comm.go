...
// Net - A miniature net package that is used to dial or listen from/to all reachable
// implants, or the server itself in the base context. This allows developers to transparently
// use an implant networking capabilities. Each command needing such network access will just
// need to grab this network interface by the SessionID, and use it as he would with the std net package.
type Net interface {
	Dial(network, hostport string) (net.Conn, error)
	DialContext(ctx context.Context, network, hostport string) (net.Conn, error)
	DialUDP(network, hostport string) (net.PacketConn, error)

	Listen(network, hostport string) (net.Listener, error)
	ListenPacket(network, hostport string) (net.PacketConn, error)
}
