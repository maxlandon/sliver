...
// ResolveBySession - Simply create an anonymous route based on a session.
func ResolveBySession(session *core.Session) (route *Route, err error) {
	...
	// Add the session comm to the route
	route.comm = Comms.GetBySession(session.ID)
