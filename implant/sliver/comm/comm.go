...
var (
	...
	serverKeyFingerprint = `{{.Config.CommServerFingerprint}}`
	serverCert           = `{{.Config.CommServerCert}}`
	serverKey            = `{{.Config.CommServerKey}}`
)
...
func InitClient(conn net.Conn) (err error) {
	...
	err = comm.setupAuthClient(constants.SliverName)
	if err != nil {
		// {{if .Config.Debug}}
...
func (comm *Comm) setupAuthClient(name string) (err error) {
